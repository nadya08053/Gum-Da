                                ...
                                <form action="{{ $action }}" method="post" id="add-edit-form" enctype="multipart/form-data">
                                                                    ...
                                                                    <div class="col-sm-4">
                                                                        <select id="years" class="form-control">
                                                                            <?php for($i = date('Y') ; $i >= date('Y', strtotime('-100 years')); $i--){ ?>
                                                                                <option value='{{ $i }}' {{ ( $user->dob && explode('/', $user->dob)[2] == $i )?'selected':'' }}>{{ $i }}</option>
                                                                            <?php } ?>
                                                                        </select>
                                                                    </div>
                                                                    <div class="col-sm-4">
                                                                        <select id="months" class="form-control">
                                                                            <?php for($i = 1; $i <= 12; $i++){$i = str_pad($i, 2, 0, STR_PAD_LEFT); $month = date('M', strtotime('2012-' . $i . '-01'));;?>
                                                                                <option value='{{ $i }}' {{ ( $user->dob && explode('/', $user->dob)[1] == $i )?'selected':'' }}>{{ $month }}</option>
                                                                            <?php } ?>
                                                                        </select>
                                                                    </div>
                                                                    <div class="col-sm-4">
                                                                        <select id="days" class="form-control">
                                                                            <?php for($i = 1; $i <= 31; $i++){$i = str_pad($i, 2, 0, STR_PAD_LEFT); ?>
                                                                                <option value='{{ $i }}' {{ ( $user->dob &&  explode('/', $user->dob)[0] == $i )?'selected':'' }}>{{ $i }}</option>
                                                                            <?php } ?>
                                                                        </select>
                                                                    </div>
                                                                </div>
                                                    ...
                                                    <div class="col-sm-4">
                                                        <select name="state" class="form-control m-b">
                                                            ...
                                                            @foreach ( $statesList as $key )
                                                                <option value="{{ $key->stateName }}" {{ ( $user->state == $key->stateName )?'selected':'' }}>{{ $key->stateName }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                    <div class="col-sm-4">
                                            ...
                                            <div class="col-sm-9">
                                                <select name="role" id="role" class="form-control m-b">
                                                    @foreach ( $selectRole as $val )
                                                        <option value="{{ $val }}"  {{ ( $user->role == $val )?'selected':'' }}>{{ $val }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="row m-b" id="select-facility" style="{{ ( $user->role != 'Admin')?'display:block':'display:none' }}">
                                            <div class="col-sm-3">
                                            ...
                                            <div class="col-sm-9">
                                                <select name="facility_id" id="facility" class="form-control m-b">
                                                    ...
                                                    @foreach ( $selectFacility as $val )
                                                        <option value="{{ $val->id }}" {{ ( $user->facility_id == $val->id)?'selected':'' }}>{{ $val->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="row m-b" id="select-trainer" style="{{ ( $user->role == 'Customer')?'display:block':'display:none' }}">
                                            <div class="col-sm-3">
                                            ...
                                            <div class="col-sm-9">
                                                <select name="trainer_id" class="form-control m-b">
                                                    ...
                                                    @foreach ( $selectTrainer as $val )
                                                        <option value="{{ $val->id }}" {{ ( $user->trainer_id == $val->id)?'selected':'' }}>{{ $val->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                    ...
                                    <div class="form-group row">
                                        <div class="col-sm-9 col-sm-offset-3">
                                            ...
                                            <a href="{{ url('/') }}/dashboard" class="btn w-xs btn-info">Cancel</a>
                                        </div>
                                    </div>
                                </form>
                            </div>
